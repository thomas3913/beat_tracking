...
#model_to_load = "models/checkpoints_02b_ASAP/model_iter_78000.pt"
...
for epoch in range(loaded_epoch, num_epochs+loaded_epoch):
    ...
    for i,datapoint in enumerate(train_loader):
        ...
        if iter % 2000 == 0:
            ...
            # Plot training loss:
            plot_value_list(loss_list,"blue","Training loss after "+str(iter)+" samples",save=True,save_name="Training_loss_ASAP")
            ...
            plot_value_list(val_loss_list,"green","Validation summary loss after "+str(len(val_loss_list))+" validation runs",save=True,save_name="Validation_loss_ASAP")
            plot_value_list(fscore_list_val_average,"red","Validation F-Score (beats) after "+str(len(fscore_list_val_average))+" validation runs",save=True,save_name="Validation_f_score_b_ASAP")
            plot_value_list(fscore_list_val_average_db,"orange","Validation F-Score (downbeats) after "+str(len(fscore_list_val_average_db))+" validation runs",save=True,save_name="Validation_f_score_db_ASAP")
            ...
            d,h,m,s = show_time(total_time)
            torch.save(model,'models/checkpoints_02b_ASAP/model_iter_'+str(iter)+'.pt')
            ...
            with open("models/checkpoints_02b_ASAP/model_iter_"+str(iter)+"_progress_info.json", 'w') as f:
